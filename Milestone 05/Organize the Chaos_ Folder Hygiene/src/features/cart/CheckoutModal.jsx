      ...
      {!isDone ? (
        <div className="text-center py-6">
          ...
          <p className="text-slate-500 max-w-xs mx-auto mb-10 leading-relaxed font-medium">
            Click the button below to complete your checkout and we'll start packing your order.
          </p>

          <div className="space-y-4">
            <Button className="w-full text-lg py-5" onClick={handleCheckout} isLoading={isSubmitting}>
              Confirm Checkout
              <ChevronRight className="w-6 h-6 ml-2" />
            </Button>

            <button onClick={onClose} className="text-slate-400 font-bold hover:text-slate-600 transition-colors py-2">
              Wait, I'm not done
            </button>
          </div>
        </div>
        ...
        <div className="text-center py-10 animate-in fade-in zoom-in duration-500">
          ...
          <p className="text-slate-500 font-medium max-w-xs mx-auto">
            Your order is on the way. You'll receive a confirmation email shortly.
          </p>
        </div>
      )}
    </Modal>
