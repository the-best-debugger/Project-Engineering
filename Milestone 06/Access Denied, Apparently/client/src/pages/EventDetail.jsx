...
const EventDetail = () => {
  ...
  const showRSVP = event.isInvited && !event.hasRSVPed;
  const showDelete = event.isCreator;
    ...
    <div className="min-h-screen bg-slate-50 pb-24">
      ...
      <main className="max-w-4xl mx-auto px-6 py-12">
        <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
          <div className="h-32 bg-blue-600 p-8 flex items-end relative overflow-hidden">
            <div className="absolute top-0 right-0 p-6 flex flex-col items-end gap-2">
              <span
                className={`px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest backdrop-blur-md shadow-2xl ${
                  event.isCreator
            ...
            <h1 className="text-3xl md:text-5xl font-black text-white leading-tight drop-shadow-sm uppercase tracking-tight">
              {event.title}
            </h1>
          </div>
          ...
          <div className="p-8 md:p-12">
            <div className="grid grid-cols-1 md:grid-cols-12 gap-10">
              <div className="md:col-span-8 space-y-10">
                <div>
                  <div className="flex items-center gap-2 text-slate-400 mb-4">
                    <Info size={16} />
                    <h2 className="text-xs font-extrabold uppercase tracking-widest leading-none">
                      Description
                    </h2>
                  </div>
                ...
                {(showRSVP || showDelete) && (
                  <div className="pt-10 flex flex-col md:flex-row gap-4 items-center border-t border-slate-100">
                    ...
                    {event.hasRSVPed && event.isInvited && (
                      <p className="text-green-600 font-bold flex items-center gap-2">
                        <CheckCircle size={20} /> You have RSVPed to this event
                      </p>
                    )}
              ...
              <div className="md:col-span-4 bg-slate-50 border border-slate-100 rounded-2xl p-6">
                <div className="flex items-center gap-2 text-slate-400 mb-4">
                  <User size={16} />
                  <h4 className="text-xs font-extrabold uppercase tracking-widest leading-none">
                    Guest List
                  </h4>
                </div>
                <div className="space-y-3">
                  {event.invitedEmails.length > 0 ? (
                    event.invitedEmails.map((email, i) => (
                      <div
                        key={i}
                        className="bg-white border border-slate-100 px-4 py-3 rounded-xl shadow-sm text-sm font-semibold text-slate-700 truncate hover:border-blue-200 transition-colors"
                      >
                        {email}
                      </div>
                    ))
        ...
        <p className="mt-8 text-center text-slate-400 text-xs font-bold leading-none tracking-widest">
          EVENT_MANAGER_SYSTEM_V.1.0_ACCESS_CONTROL_ENABLED
        </p>
      </main>
    </div>
  );
