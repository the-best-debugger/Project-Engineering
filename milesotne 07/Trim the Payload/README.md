...
## Fixes applied (solution)

| Issue | Fix |
|-------|-----|
| N+1 queries | `server/src/routes/orders.ts` — one `findMany` with nested `select` |
| No pagination | `?page` & `?limit` with `skip`/`take` + metadata |
| Over-fetching | Explicit `select` — only fields the table uses |
| Event loop block | Removed sync busy-wait `map` |
| No compression | `compression()` middleware in `server/src/index.ts` |

See **BASELINE.md** for before/after metrics. Enable query logging: `LOG_QUERIES=true npm run dev` in `server/`.
